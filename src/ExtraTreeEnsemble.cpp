 ...
 *   a with this program; if not, write to the                         *
...
ExtraTreeEnsemble::ExtraTreeEnsemble(unsigned int input_size,
    ...
    Regressor("ExtraTrees", input_size, output_size)
{
    ...
    for (unsigned int i = 0; i < mNumTrees; i++) {
        mEnsemble.push_back(new ExtraTree(mInputSize, mOutputSize, mNumSplits, mNMin, mScoreThreshold, mLeafType));
    }
...
{
    return new ExtraTreeEnsemble(mInputSize, mOutputSize, mNumTrees, mNumSplits, mNMin, mScoreThreshold, mLeafType);
}
...
multimap<float, unsigned int> ExtraTreeEnsemble::EvaluateFeatures(float initial_variance, float min_threshold)
{
...
//   cout << "Average result" << endl;
    ...
    do {
        --rank_it;
        cout << 100 * rank_it->first / initial_variance << " %\t" << rank_it->first / cum_sum << " %\t" << rank_it->second << "\t" << rank_it->first << endl;

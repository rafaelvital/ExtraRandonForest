...
Regressor::Regressor (string type, unsigned int input_size, unsigned int output_size)
    : mType(type),mInputSize(input_size),mOutputSize(output_size)
{}
...
map<string, float> Regressor::ComputePerfMetrics(Dataset* data)
{
    ...
    float raae = 0.0, rsquare = 0.0, rmae = 0.0, mse = 0.0,
          m = 0.0, m2 = 0.0, variance = 0.0, y_ps = -1e6, y_po = -1e6;
    Dataset::iterator it;
    for (it = data->begin(); it != data->end(); ++it) {
        Tuple result(mOutputSize);
        Evaluate((*it)->GetInputTuple(), result);
        ...
        //std::cout << "Erro: " << e_i << " output: " << output << " result: " <<  result[0]<< std::endl;
        mse += e_i * e_i;
...
float Regressor::ComputeResiduals(Dataset* data,
                                  set<unsigned int> inputs,
...
map<string, float> Regressor::CrossValidate(Dataset* data,

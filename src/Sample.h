...
class Sample
{
    public:
        ...
        void setInput(unsigned pos,float value);
        void setOutput(unsigned pos,float value);
...
inline void Sample::setInput(const unsigned pos,float value){
    mpInput->at(pos) = value; //strange bug, when was  mpInput[pos] = value the program crashed
}

inline void Sample::setOutput(const unsigned pos,float value){
    mpOutput->at(pos) = value;
}

...
float* Dataset::GetMatrix()
{
     unsigned int num_cols = mInputSize + mOutputSize;
    float* matrix = (float*)malloc(sizeof(float) * size() * num_cols);
...
Dataset* Dataset::GetReducedDataset(unsigned int size, unsigned* samples){
	Dataset* ds = new Dataset(mInputSize, mOutputSize);
	ds->resize(size);
	for(unsigned i = 0; i < size; i++){
		ds->at(i) =  this->at(samples[i]);
	}
	return ds;
}
...
void Dataset::GetTrainAndTestDataset(unsigned int num_partitions,
                                  ...
                                  vector<pair<float, float> >& output_parameters)
{
    ...
    for (unsigned int i = 0; i < mInputSize; i++) {
        input_parameters.push_back(pair<float, float>((*begin())->GetInput(i), (*begin())->GetInput(i)));
    }
    for (unsigned int i = 0; i < mOutputSize; i++) {
        output_parameters.push_back(pair<float, float>((*begin())->GetOutput(i), (*begin())->GetOutput(i)));
    }
    ...
    for (it = begin(); it != end(); ++it) {
        ...
        for (unsigned int i = 0; i < mInputSize; i++) {
            (*ti)[i] = 2 * ((*it)->GetInput(i) - input_parameters[i].first) / (input_parameters[i].second - input_parameters[i].first) - 1;
        }
        Tuple* to = new Tuple(mOutputSize);
        for (unsigned int i = 0; i < mOutputSize; i++) {
            (*to)[i] = 2 * ((*it)->GetOutput(i) - output_parameters[i].first) / (output_parameters[i].second - output_parameters[i].first) - 1;
        }
        ds->AddSample(ti, to);
    }

...
ExtraTree::ExtraTree(unsigned int input_size,
                     unsigned int output_size,
                     int k,
                     int nmin,
                     float score_th,
                     LeafType leaf) : Tree("ExtraTree", input_size, output_size)
{
    root = NULL;
    mNumSplits = k;
    mNMin = nmin;
...
{
    out << mNumSplits << " " << mNMin << " " << mInputSize << " " << mOutputSize << " " << mLeafType << endl;
    out << *root;
}
...
// 		cout << "size = " << size << endl;
    ...
    for( unsigned i = 0; i < candidates_size ; ++i){
         unsigned int r = (rand() % (selectable-i)) +1;
...
//    cout << "Best: " << bestattribute << " " << bestscore << " " << mScoreThreshold << endl;
    if (bestscore < mScoreThreshold) {
        ...
        if (mFeatureRelevance != NULL) {
            float variance_reduction = VarianceReduction(ds, &bestSl, &bestSr) * ds->size() * ds->Variance();
...
float ExtraTree::VarianceReduction(Dataset* ds, Dataset* dsl, Dataset* dsr)
{
    ...
    if (ds->size() == 0 || ds->Variance() == 0.0) {
        ...
        return 1 - ((float)corr_fact_dsl * dsl->size() * dsl->Variance() + (float)corr_fact_dsr * dsr->size() * dsr->Variance()) / ((float)corr_fact_ds * ds->size() * ds->Variance());
    }
}
